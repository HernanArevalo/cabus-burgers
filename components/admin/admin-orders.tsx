                        ...
                        {(Object.entries(statusConfig) as [OrderStatus, typeof statusConfig[OrderStatus]][]).map(
                          ([statusKey, cfg]) => {
                            ...
                            return (
                              ...
                              >
                                <StatusItemIcon className="w-3.5 h-3.5" style={{ color: cfg.color }} />
                                <span>{cfg.label}</span>
                              </DropdownMenuItem>
                            )
                          }
                  ...
                  {detailOrder.status !== "CANCELADO" && detailOrder.status !== "ENTREGADO" && (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        {(Object.entries(statusConfig) as [OrderStatus, typeof statusConfig[OrderStatus]][]).map(
                          ([statusKey, cfg]) => {
                            if (statusKey === detailOrder.status) return null
                            const StatusItemIcon = cfg.icon
                            return (
                              <DropdownMenuItem
                                key={statusKey}
                                onClick={() => handleStatusChange(detailOrder.id, statusKey)}
                                className="flex items-center gap-2 cursor-pointer"
                              >
                                <StatusItemIcon className="w-4 h-4" style={{ color: cfg.color }} />
                                <span>{cfg.label}</span>
                              </DropdownMenuItem>
                            )
                          }
